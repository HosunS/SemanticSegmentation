...
class ESDDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        #split if we are only want to augment the training data
        self.transform_train = torchvision_transforms.Compose(transform_list)
        self.transform_val = torchvision_transforms.Compose(preprocess_list)
    ...
    def prepare_data(self) -> None:
        ...
        if not self.processed_dir.exists() or not any(self.processed_dir.iterdir()):
            ...
            # We have now created the train test split. We are going to subtile and save these into the self.train_dir and self.val_dir
            ...
            for tile_dir in tqdm(list(tile_dirs_train), desc="Processing train tiles"):
                ...
                data_array_list, ground_truth = self.load_and_preprocess(tile_dir)
                # create a subtile, passing the data array list, gt data array, and the slice size
                subtile = Subtile(data_array_list, ground_truth, self.slice_size)
    ...
    def setup(self, stage: str) -> None:
        ...
        if stage == "fit":
            ...
            #TEST change transform only for val to not augment val data
            train_dataset = ESDDataset(self.train_dir, satellite_type_list= self.satellite_type_list, transform=self.transform)

            # create the val ESDDataset (the processed_dir will be the val_dir)
            val_dataset = ESDDataset(self.val_dir, satellite_type_list= self.satellite_type_list, transform=self.transform)
    ...
    def train_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        # and collate_fn
        return torch.utils.data.DataLoader(self.train_dataset, num_workers=self.num_workers, batch_size=self.batch_size, collate_fn=collate_fn,persistent_workers=True)
    ...
    def test_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        # create the torch.utils.data.Dataloader for the train_dataset, passing the batch size
        # and collate_fn
        return torch.utils.data.DataLoader(self.test_dataset, num_workers=self.num_workers, batch_size=self.batch_size, collate_fn=collate_fn,persistent_workers=True)
    ...
    def val_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        # and collate_fn
        return torch.utils.data.DataLoader(self.val_dataset,num_workers=self.num_workers, batch_size=self.batch_size, collate_fn=collate_fn, persistent_workers=True)