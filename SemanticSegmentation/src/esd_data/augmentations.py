...
class Blur(object):
    ...
    def __call__(self, sample: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        new_img = apply_per_band(img, lambda x:cv2.blur(x, ksize=(kernel,kernel)))

        # return the {X : img, y : mask}
        return {"X": new_img, "y": mask}
...
class AddNoise(object):
    ...
    def __call__(self, sample):
        ...
        random_standard_deviation = random.uniform(0, self.std_lim)
        ...
        noise = np.random.normal(self.mean, random_standard_deviation, img.shape)
        
        # add the noise to the img and clip between 0 and 1
        new_img = np.clip(img+noise,0,1)
...
class RandomHFlip(object):
    ...
    def __call__(self, sample):
        ...
        if (random.random()<=self.p):
            # apply per band the cv2.flip function (you can pass it as a lambda)
            new_img = apply_per_band(img, lambda x: cv2.flip(x,1))
            # also flip the mask (not per band, just directly on the mask)
