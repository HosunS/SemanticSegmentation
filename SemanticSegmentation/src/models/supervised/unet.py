...
class DoubleConvHelper(nn.Module):
    ...
    def forward(self, x):
        ...
        # conv1
        # print("X shape before doubleconv forward", x.shape)
        ...
        # relu
        x = self.relu(x)
        # print("X shape after doubleconv forward", x.shape)
        return x
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        # print("X shape before encoder forward", x.shape)
        x = torch.nan_to_num(x)
        # maxpool2d
        x = self.maxpool(x)
        # doubleconv
        x= self.double_conv(x)
        # print("X shape after encoderforward", x.shape)
...
class Decoder(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        # print("X shape before decoder forward", x.shape)
        # step 6 Pass the concatenated tensor through a doubleconvhelper
        x = self.double_conv(x)
        # print("X shape after decoder forward", x.shape)
...
class OutConv(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("X shape before outconv forward", x.shape)
        x = self.conv(x)
        # print("X shape after outconv forward", x.shape)
        return x
...
class UNet(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, n_encoders: int = 2,
        ...
        self.out_conv = OutConv(embedding_size, out_channels)
        # create a doubleconvhelper
    ...
    def forward(self, x):
        ...
        residuals.append(x)
        # for each encoder
        # print("X shape before unet forward", x.shape)
        ...
        x = self.pool(x)
        # print("X shape after unet forward", x.shape)
        return x
    

# model = UNet(in_channels=3, out_channels=4)
# output = model(torch.randn(12, 3, 200, 200))
# print(output.shape)